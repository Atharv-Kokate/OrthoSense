...
def evaluate_patient_adaptation(db: Session, patient_id: int, exercise_type: str = "squat"):
    ...
    if form_score >= 85.0 and actual_reps >= target_reps and avg_rom >= (target_rom - 5.0):
        # Progress logic: 
        # Alternating progression strategy - either push ROM further OR increase rep endurance.
    ...
    elif form_score < 60.0 or avg_rom < (target_rom - 15.0):
        # Regress logic: Safety first. Ease off the targeted ROM.
        if target_rom > 60.0:
            new_target_rom -= 10.0
        elif target_reps > 5:
            new_reps_per_set -= 2
