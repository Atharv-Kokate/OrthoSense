...
'''
with open('frontend/src/pages/doctor/ProgramBuilder.jsx', 'wb') as f:
    f.write(builder_code.encode('utf-8'))

with open('frontend/src/pages/patient/PatientDashboard.jsx', 'r', encoding='utf-8') as f:
    text = f.read()

# Replace the mangled fetch call with backticks and correctly placed variables
if 'http://localhost:8000/api/patients//journey' in text:
    text = text.replace('const res = await fetch(http://localhost:8000/api/patients//journey);', "const res = await fetch(http://localhost:8000/api/patients//journey);")

# Next replace the navigate to support interpolation again
if "navigate(/patient/session/" in text:
    target_navigate = "onClick={() => navigate(/patient/session/)}"
    text = text.replace("navigate(/patient/session/)", "navigate(/patient/session/)")

# We should do regex replace to be completely safe against mangled {phase.exercises...}
import re
text = re.sub(r'navigate\(/patient/session/.*?\)', "navigate(/patient/session/)", text)

with open('frontend/src/pages/patient/PatientDashboard.jsx', 'wb') as f:
    f.write(text.encode('utf-8'))
    
print("Fixed UTF-8 and Syntax")